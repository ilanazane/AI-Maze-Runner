...
def fireAstarM(maze, q, video, show_final):
    ...
    fringe = []
    fringe.append((i, j, counter + Manhattan(maze, i, j, q)))
    ...
    while solved == False and onFire == False:
        ...
        counter = current[2] - Manhattan(maze, i, j, q) + 1
        ...
        if j - 1 < 0:
            ...
            if maze[i][j - 1] == 1 or maze[i][j - 1] == 0.5 or maze[i][j - 1] == 0.75:
                ...
                if (i, j - 1, counter + Manhattan(maze, i, j - 1,q)) in fringe:
                    pass
                else:
                    #if the fringe was empty (i.e. the first move), just add the first child to fringe
                    if len(fringe) == 0:
                        prev[(i, j - 1, counter + Manhattan(maze, i, j - 1,q))] = (i, j, counter - 1 + Manhattan(maze, i, j,q))
                        fringe.append((i, j - 1, counter + Manhattan(maze, i, j - 1,q)))
                    #if the fringe is not empty, check the f(n) value and compare with items in fringe
                    else:

                        for x in range(len(fringe)-1,-1,-1):
                            if counter + Manhattan(maze, i, j - 1,q) > fringe[x][2]:
                                if x == 0:
                                    fringe.insert(0, (i, j - 1, counter + Manhattan(maze, i, j - 1,q)))
                                    prev[(i, j - 1, counter + Manhattan(maze, i, j - 1,q))] = (i, j, counter - 1 + Manhattan(maze, i, j,q))
                                else:
                                    pass
                            else:
                                fringe.insert(x + 1, (i, j - 1, counter + Manhattan(maze, i, j - 1,q)))
                                prev[(i, j - 1, counter + Manhattan(maze, i, j - 1,q))] = (i, j, counter - 1 + Manhattan(maze, i, j,q))
                                break
        ...
        if i - 1 < 0:
            ...
            if maze[i - 1][j] == 1 or maze[i - 1][j] == 0.5 or maze[i - 1][j] == 0.75:
                ...
                #check if already in fringe
                if (i - 1, j, counter + Manhattan(maze, i - 1, j,q)) in fringe:
                    pass
                else:
                    #if the fringe was empty (i.e. the first move), just add the first child to fringe
                    if len(fringe) == 0:
                        prev[(i - 1, j, counter + Manhattan(maze, i - 1, j,q))] = (i, j, counter - 1 + Manhattan(maze, i, j,q))
                        fringe.append((i - 1, j, counter + Manhattan(maze, i - 1, j, q)))
                    #if the fringe is not empty, check the f(n) value and compare with items in fringe
                    else:
                        for x in range(len(fringe)-1,-1,-1):
                            if counter + Manhattan(maze, i - 1, j, q) > fringe[x][2]:
                                if x == 0:
                                    fringe.insert(0, (i - 1, j, counter + Manhattan(maze, i - 1, j, q)))
                                    prev[(i - 1, j, counter + Manhattan(maze, i - 1, j, q))] = (i, j, counter - 1 + Manhattan(maze, i, j, q))
                                else:
                                    pass
                            else:
                                fringe.insert(x + 1, (i - 1, j, counter + Manhattan(maze, i - 1, j, q)))
                                prev[(i - 1, j, counter + Manhattan(maze, i - 1, j, q))] = (i, j, counter - 1 + Manhattan(maze, i, j, q))
                                break
...
#fireAstarM(firegrid(10, 0.2), video = True, show_final = False, q = .5)
